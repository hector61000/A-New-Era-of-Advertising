...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($phone) || empty($password)) {
        ...
        if (mysqli_num_rows($check_result) > 0) {
            ...
            // إضافة المستخدم
            $sql = "INSERT INTO users (phone, password, role) VALUES ('$phone', '$hashed_password', '$role')";
            
            if (mysqli_query($conn, $sql)) {
                $success = "تم إضافة المستخدم بنجاح";
            } else {
                $error = "حدث خطأ أثناء إضافة المستخدم: " . mysqli_error($conn);
            }
        }
    }
}
...
<head>
    ...
    <title>إضافة مستخدم جديد - منصة الإعلانات</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="container mx-auto p-6">
        <div class="bg-white rounded-lg shadow-lg p-6">
            <h2 class="text-xl font-bold mb-6">إضافة مستخدم جديد</h2>
            ...
            <form method="POST" action="">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="password" name="password" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" required>
                    <p class="text-sm text-gray-500 mt-1">كلمة المرور ستظهر بوضوح. يمكنك إنشاء كلمة مرور عشوائية أو اختيار كلمة سهلة التذكر للمستخدم.</p>
                </div>
